...
class view
{
    ...
    static public $config_status = false;
    ...
    public function __construct(array $config = [])
    {
        ...
        //加载静态资源
        $this->data['STATIC'] = './public/static/' . MODULE ;
        $this->data['COMMON'] = './public/static/common';
    ...
    public function display($template, $vars = [], $config = [])
    {
        ...
        if ($template) {
            ...
            ob_implicit_flush(0);
            // 读取编译存储
            $this->read($cacheFile, $this->data); //TODO
            // 获取并清空缓存
    ...
    private function parseTemplateFile($template)
    {
        ...
        if ($this->config['template'] !== false && !empty($this->config['template'])) {
            $template = $this->config['template'] . '/' . $template;
        }

        if (false === strpos($template, '.')) {
            // 跨模块支持
            $template = strpos($template, '@') ?
                APP_ROOT . str_replace('@', '/' . basename($this->config['view_path']) . '/', $template) . $this->config['view_suffix'] :
